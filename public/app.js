...
function restoreQueue() {
  ...
  document.title = `${s.title} • ARA Music`;
...
setInterval(() => {
  ...
  if (!isPreviewing()) updateLyARAighlight(cur);
...
let lastLyricIdx = -1;
function updateLyARAighlight(cur) {
  const L = Player.lyrics;
...
function backupLibrary() {
  ...
  a.href = url;
  a.download = `ARA-music-backup-${new Date().toISOString().slice(0, 10)}.json`;
  a.rel = 'noopener';
  ...
  inp.onchange = () => {
    ...
    reader.onload = () => {
      try {
        ...
        if (!d || (d.app !== 'ARA-music' && d.app !== 'smw')) throw new Error('Not a ARA Music backup');
